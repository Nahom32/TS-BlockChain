...
const initP2PServer = (p2pPort) => {
    const server = new ws_1.WebSocket.Server({ port: p2pPort });
...
const responseChainMsg = () => ({
    'type': MessageType.RESPONSE_BLOCKCHAIN, 'data': JSON.stringify((0, block_functions_1.getBlockChain)())
});
const responseLatestMsg = () => ({
    'type': MessageType.RESPONSE_BLOCKCHAIN,
    'data': JSON.stringify([(0, block_functions_1.getLatestBlock)()])
});
...
const handleBlockchainResponse = (receivedBlocks) => {
    ...
    if (!(0, block_functions_1.isValidBlockStructure)(latestBlockReceived)) {
        console.log('block structuture not valid');
        return;
    }
    const latestBlockHeld = (0, block_functions_1.getLatestBlock)();
    if (latestBlockReceived.index > latestBlockHeld.index) {
        console.log('blockchain possibly behind. We got: '
            + latestBlockHeld.index + ' Peer got: ' + latestBlockReceived.index);
        if (latestBlockHeld.hash === latestBlockReceived.previousHash) {
            if ((0, block_functions_1.addBlockToChain)(latestBlockReceived)) {
            ...
            (0, block_functions_1.replaceChain)(receivedBlocks);
        }
    }
    else {
...
const connectToPeers = (newPeer) => {
    const ws = new ws_1.WebSocket(newPeer);
    ws.on('open', () => {
        initConnection(ws);
    });
    ws.on('error', () => {
